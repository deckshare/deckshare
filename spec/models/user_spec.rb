...
RSpec.describe User, type: :model do
  ...
  describe "#add_pokemon_card!" do
    ...
    context "with an existing card id" do
      before { FactoryBot.create(:deckshare_pokemon_card, user:, card_id:) }
  ...
  describe "#remove_pokemon_card!" do
    ...
    context "with a quantity greater than card.quantity" do
      ...
      it { expect { remove_pokemon_card! }.to raise_error(RangeError, "#{initial_quantity + 1} not in range 1..#{initial_quantity}") }
    end

    context "with a quantity of 0" do
      let(:quantity) { 0 }

      it { expect { remove_pokemon_card! }.to raise_error(RangeError, "#{quantity} not in range 1..#{initial_quantity}") }
    end
  end
end
